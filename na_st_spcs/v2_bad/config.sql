...
CREATE PROCEDURE config.reference_callback(ref_name STRING, operation STRING, ref_or_alias STRING)
 ...
 AS $$
    DECLARE
        retstr STRING;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: reference_callback: ref_name=' || ref_name || ' operation=' || operation);
        CASE (operation)
            WHEN 'ADD' THEN
...
CREATE PROCEDURE config.grant_callback(privs ARRAY)
    ...
    BEGIN
        IF (ARRAY_CONTAINS('CREATE COMPUTE POOL'::VARIANT, :privs)) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: creating all compute pools');
            CALL config.create_all_compute_pools() INTO :retstr;
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: compute pools: ' || :retstr);
        END IF;
        IF (ARRAY_CONTAINS('CREATE WAREHOUSE'::VARIANT, :privs)) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: creating all warehouses');
            CALL config.create_all_warehouses() INTO :retstr;
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: warehouses: ' || :retstr);
        END IF;
        -- Whenever grants are added, see if we can start services that aren't started already
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: creating all services');
        CALL config.create_all_services() INTO :retstr;
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: services: ' || :retstr);
        RETURN retstr;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: grant_callback: EXCEPTION: ' || SQLERRM);
    END;
    $$;
...
CREATE PROCEDURE config.version_initializer()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: version_initializer: initializing');
...
CREATE OR REPLACE PROCEDURE config.compute_pool_exists(name STRING)
    ...
    BEGIN
        ...
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: compute_pool_exists: Service found? ' || b);
        RETURN b;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: compute_pool_exists: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.warehouse_exists(name STRING)
    ...
    BEGIN
        ...
        RETURN false;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: warehouse_exists: ERROR: ' || SQLERRM);
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.service_exists(name STRING)
    ...
    BEGIN
        ...
        IF (ct > 0) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: service_exists: Service found');
            RETURN true;
        END IF;
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: service_exists: Did not find service');
        RETURN false;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: service_exists: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.service_suspended(name STRING)
    ...
    BEGIN
        ...
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: service_suspended: Service suspended? ' || b);
        RETURN b;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: service_suspended: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.create_all_compute_pools()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_all_compute_pools: creating all compute pools');
        CALL config.permissions_and_references(ARRAY_CONSTRUCT('CREATE COMPUTE POOL'),
                                            ARRAY_CONSTRUCT()) INTO :b;
        IF (NOT b) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_all_compute_pools: Insufficient permissions');
            RETURN false;
        END IF;
...
CREATE OR REPLACE PROCEDURE config.drop_all_compute_pools()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_all_compute_pools: dropping compute pools');
...
CREATE OR REPLACE PROCEDURE config.create_compute_pool_stpool()
    ...
    DECLARE
        ...
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_compute_pool_stpool: creating compute pool ' || name);
        CALL config.permissions_and_references(ARRAY_CONSTRUCT('CREATE COMPUTE POOL'),
                                            ARRAY_CONSTRUCT()) INTO :b;
        IF (NOT b) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_compute_pool_stpool: Insufficient permissions');
            RETURN false;
        END IF;
        CREATE COMPUTE POOL IF NOT EXISTS Identifier(:name)
            ...
            AUTO_RESUME = TRUE;
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_compute_pool_stpool: compute pool ' || name || ' dropped');
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_compute_pool_stpool: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.drop_compute_pool_stpool()
    ...
    DECLARE
        name STRING DEFAULT config.app_prefix('stpool');
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_compute_pool_stpool: dropping compute pool ' || name);
        DROP COMPUTE POOL IF EXISTS Identifier(:name);
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_compute_pool_stpool: compute pool ' || name || ' dropped');
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_compute_pool_stpool: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.create_all_warehouses()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_all_warehouses: creating all warehouses');
        CALL config.permissions_and_references(ARRAY_CONSTRUCT('CREATE WAREHOUSE'),
                                            ARRAY_CONSTRUCT()) INTO :b;
        IF (NOT b) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_all_warehouses: Insufficient permissions');
            RETURN false;
        END IF;
...
CREATE OR REPLACE PROCEDURE config.drop_all_warehouses()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_all_warehouses: dropping all warehouses');
...
CREATE OR REPLACE PROCEDURE config.create_warehouse_stwh()
    ...
    DECLARE
        ...
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_warehouse_stwh: creating warehouse ' || name);
        CALL config.permissions_and_references(ARRAY_CONSTRUCT('CREATE WAREHOUSE'),
                                            ARRAY_CONSTRUCT()) INTO :b;
        IF (NOT b) THEN 
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_warehouse_stwh: Insufficient privileges');
            RETURN false;
        END IF;

        CREATE WAREHOUSE IF NOT EXISTS Identifier(:name) WITH WAREHOUSE_SIZE='XSMALL';
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_warehouse_stwh: warehouse ' || name || ' created');
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_warehouse_stwh: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.drop_warehouse_stwh()
    ...
    DECLARE
        name STRING DEFAULT config.app_prefix('stwh');
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_warehouse_stwh: dropping warehouse ' || name);
        DROP WAREHOUSE IF EXISTS Identifier(:name);
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_warehouse_stwh: warehouse ' || name || ' dropped');
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_warehouse_stwh: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.create_all_services()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_all_services: creating all services');
...
CREATE OR REPLACE PROCEDURE config.drop_all_services()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_all_services: dropping all services');
...
CREATE OR REPLACE PROCEDURE config.suspend_all_services()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: suspend_all_services: suspending all services');
...
CREATE OR REPLACE PROCEDURE config.resume_all_services()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: resume_all_services: resuming all services');
...
CREATE OR REPLACE PROCEDURE config.upgrade_all_services()
    ...
    DECLARE
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_all_services: upgrading all services');
        ...
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_all_services: ERROR: ' || SQLERRM);
        RAISE;
    END
    $$;
    GRANT USAGE ON PROCEDURE config.resume_all_services() TO APPLICATION ROLE app_admin;
CREATE OR REPLACE PROCEDURE config.create_service_st_spcs()
    ...
    DECLARE
        ...
        b BOOLEAN;
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: starting');
        ...
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: checking if we have all permissions and references');
        CALL config.permissions_and_references(ARRAY_CONSTRUCT('BIND SERVICE ENDPOINT'),
                                            ARRAY_CONSTRUCT('EGRESS_EAI_WIKIPEDIA')) INTO :b;
        IF (NOT b) THEN
            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: Insufficient permissions');
            RETURN false;
        END IF;

        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: starting service');
        ...
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: Command: ' || q);
        EXECUTE IMMEDIATE q;


        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: waiting on service start');
        SELECT SYSTEM$WAIT_FOR_SERVICES(300, 'APP_PUBLIC.ST_SPCS');
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: granting usage');
        GRANT USAGE ON SERVICE app_public.st_spcs TO APPLICATION ROLE app_user;

        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: finished!');
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: create_service_st_spcs: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.drop_service_st_spcs()
    ...
    AS $$
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_service_st_spcs: dropping service ST_SPCS');

        DROP SERVICE IF EXISTS app_public.st_spcs;
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: drop_service_st_spcs: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.suspend_service_st_spcs()
    ...
    AS $$
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: suspend_service_st_spcs: suspending service ST_SPCS');

        ALTER SERVICE IF EXISTS app_public.st_spcs SUSPEND;
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: suspend_service_st_spcs: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.resume_service_st_spcs()
    ...
    AS $$
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: resume_service_st_spcs: resuming service ST_SPCS');

        ALTER SERVICE IF EXISTS app_public.st_spcs RESUME;
        RETURN true;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: resume_service_st_spcs: ERROR: ' || SQLERRM);
        RETURN false;
    END
    $$;
...
CREATE OR REPLACE PROCEDURE config.upgrade_service_st_spcs()
    ...
    BEGIN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_service_st_spcs: upgrading service ST_SPCS');
        ...
        IF (b) THEN
            ...
            -- MIMIC AN UPGRADE FAILURE
            RAISE SIMULATED_ST_SERVICES_EXCEPTION;

            SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_service_st_spcs: waiting on service start');
            SELECT SYSTEM$WAIT_FOR_SERVICES(300, 'APP_PUBLIC.ST_SPCS');

            IF (suspended) THEN
                SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_service_st_spcs: re-suspending service');
        ...
        END IF;
    EXCEPTION WHEN OTHER THEN
        SYSTEM$LOG_INFO('!BAD! NA_ST_SPCS: upgrade_service_st_spcs: ERROR: ' || SQLERRM);
        RAISE;
    END
    $$;
    GRANT USAGE ON PROCEDURE config.resume_service_st_spcs() TO APPLICATION ROLE app_admin;
